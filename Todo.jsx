...
const Todo = memo(() => {
    ...
    return Object.keys(toDos).length === 0
      ...
      : Object.keys(toDos).map((ID) => (
          <ToDoListDiv key={ID}>
            ...
            <StyledButton type="link" onClick={deleteToDo.bind(null, ID)}>
              <CloseSquareTwoTone />
            </StyledButton>
          </ToDoListDiv>
        ));
  ...
  return (
    <>
      <GlobalStyle />
      <P>
        <A href="https://annyhpk.github.io/reactToDo/">Todo List</A>
      </P>
      <Div>
        <Form onSubmit={addToDo}>
          ...
          />
          <StyledSubmitButton onClick={addToDo}>
            <EnterOutlined />
          </StyledSubmitButton>
        </Form>
        <ToDoBack>{renderTodo()}</ToDoBack>
      </Div>
    </>
  );
});
